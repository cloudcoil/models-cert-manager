...
> [!WARNING]  
Models are generated from cert-manager **1.21.1** using **Cloudcoil 0.7**. Run `uv sync` followed by `make gen-models` to regenerate them from the pinned upstream schema. Cloudcoil infers resource names and normalizes Kubernetes schemas without project-specific patches.
